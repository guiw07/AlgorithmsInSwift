...
for movies in movieTitleArray {
    print("\(movies) with Puppets!")
}
...
for rent in oldMountainViewRent {
    newMountainViewRent.append((rent * 0.20) + rent);
}
for rent in newMountainViewRent {
    print("New Rent: \(rent)")
}
...
for (name, number) in rapperPhoneNumbers {
    ...
    let index5 = number.index(number.startIndex, offsetBy: 5)
    
    let mySubstring = number.prefix(5)
    var myString = String(mySubstring)
    if myString == "(415)" {
        rapperPhoneNumbers[name] = "(510)" + String(number.suffix(from: index5))
    }
}
...
case (.scissors,.paper):
    resultsMessage = "Scissors cut Paper."
default:
    resultsMessage = "Nothing Happens"
}
