...
func combineLastCharacters(Of someWords:[String]) -> String {
    var newString = ""
    
    for word in someWords {
            newString.append(lastCharacter(Of: word))
        }
    return newString
}

print(combineLastCharacters(Of: nonsenseArray))
//: __Problem 3__
//:
...
func removeDirtyWords(Of someWords:[String]) -> [String] {
    var newArray = [String]()
    for word in someWords {
        if word.count != 4{
...
let newArray = removeDirtyWords(Of: dirtyWordsArray)
...
class MovieArchive {
    func filterByDirector(from director: String, from movies:[String:String]) -> [String] {
        var filteredMovies = [String]()
        for (movie, directorName) in movies {
...
print(myMovies.filterByDirector(from:"Christopher Nolan", from: movies))
